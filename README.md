...
The LC25000 dataset is a popular resource with more than 650 works citing it [(142 according to Semantic Scholar on June 28th, 2024; 650+ as of June 2026)](https://www.semanticscholar.org/paper/Lung-and-Colon-Cancer-Histopathological-Image-Borkowski-Bui/6c5a72aaa3c29d52d46ef904f15719478b6cdfc2).
...
## Using LC25000 Properly on Kaggle

The `javaidahmadwani/lc25000` dataset on Kaggle is one of the most popular downloads of LC25000, but its provided Train/Test split was made randomly over augmented images — meaning ~98% of tile-groups are contaminated across the split boundary.

The [`kaggle/`](./kaggle/) directory in this repository contains everything needed to fix this:

- **`lc25000_image_groups.csv`** — a consolidated CSV assigning each of the 25,000 images to its original tile group (`group_id`). Upload this as a small Kaggle Dataset.
- **`lc25000-clean-proper-splits.ipynb`** — a Kaggle notebook that loads the group annotations, quantifies the leakage in the provided split, and demonstrates group-aware splitting with `sklearn.model_selection.GroupShuffleSplit`.

See [`kaggle/README.md`](./kaggle/README.md) for publishing instructions (Kaggle CLI, dataset/kernel metadata).
